...
\author{SKM$^{1}$ and AB$^{2}$% <-this % stops a space
...
\begin{enumerate}
	...
	\begin{bmatrix}
	 ...
	 \label{TF}
	\end{equation}
	Note that the dependence of the model on time shift operator $q^{-1}$ is not shown for ease of notation. The signal $w(t)$ is interpreted as an exogenous disturbance signal, which causes the model uncertainty. The state-space equivalent of this model is used in the robust reference governor that was previously discussed. \\
	...
	\end{matrix}
	\end{equation*}
	The following subsection discusses a technique to calculate upper and lower bounds $w_{min}$ and $w_{max}$ on the disturbance signal $w(t)$, which are explicitly provided to the robust reference governor for the calculation of $\mathbb{O}_{\infty}$.
