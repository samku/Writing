...
\begin{enumerate}
	...
	The robust reference governer is a signal regulator which alters the command input such that the system robustly satisfies constraints. It does so by utilizing knowledge of disturbances acting on the system. A schematic of the control system is shown in Fig. \ref{fullloop}. The $MPC$ controller utilizes the model $\mathbb{M}'_{P}$, and the reference governer $RG$ uses the model $\mathbb{M}'_{PD}$, which is the state space representation of \eqref{TF}. This is written as :
	\begin{equation*}
	\begin{matrix}
	\gamma(t+1) = A_\gamma\gamma(t) + B_\gamma \begin{bmatrix}v(t) \\ w(t) \end{bmatrix} \vspace{2pt}\\
	\begin{bmatrix}y(t)\\u(t)\end{bmatrix} = C_\gamma\gamma(t) + D_\gamma\begin{bmatrix}v(t) \\ w(t) \end{bmatrix}
	\end{matrix}
	\end{equation*}
	\\
	\begin{figure}[h]
		...
		\label{fullloop}
	\end{figure} \\
	The reference governer $RG$ solves, at each time instant, the optimization problem:
